...
def vytvor(name,debug,download):
    ...
    for customer in invoice_set.customers:
        invoice_record = api_session.create_invoice(customer=customer,invoice_set=invoice_set)
        click.echo(click.style(f'Založená faktúra pod číslom {invoice_record.cislo_faktury}',fg='green'))
